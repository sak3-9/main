...
const TABS: { key: FilterKey; label: string }[] = [
  ...
  { key: 'done', label: '完了' },
  { key: 'all', label: 'すべて' },
  { key: 'assigned_me', label: 'わたし担当' },
  { key: 'assigned_partner', label: 'パートナー担当' },
...
export default function Page() {
  ...
  const doneCount = tasks.filter((t) => t.status === 'done' && !t.is_archived).length;
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-5">
        <form onSubmit={signInMagic} className="w-full max-w-md rounded-2xl border border-purple-400/30 bg-panel p-6 space-y-4 shadow-xl">
          ...
          <p className="text-sm text-purple-200">あなたとパートナーだけが使える、やさしいタスク管理です。</p>
          <input
        ...
        <section className="rounded-2xl border border-purple-500/30 bg-panel p-4">
          ...
          <div className="mb-4 flex gap-2 overflow-auto pb-1">
            {TABS.map((tab) => (
              ...
              >
                {tab.label}
                {tab.key === 'done' && (
                  <span className="ml-2 rounded-full bg-black/30 px-2 py-0.5 text-xs text-white">{doneCount}</span>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <div className="rounded-xl border border-purple-500/30 bg-black/20 p-3">
              ...
              {boardCols.urgent.map((t) => <TaskCard key={t.id} t={t} me={user.id} onSelect={setSelectedId} onToggleDone={toggleDone} onPatch={patchTask} partnerId={partnerId} />)}
            </div>
            <div className="rounded-xl border border-purple-500/30 bg-black/20 p-3">
              ...
              {boardCols.open.map((t) => <TaskCard key={t.id} t={t} me={user.id} onSelect={setSelectedId} onToggleDone={toggleDone} onPatch={patchTask} partnerId={partnerId} />)}
            </div>
            <div className="rounded-xl border border-purple-500/30 bg-black/20 p-3">
              ...
              {boardCols.done.map((t) => <TaskCard key={t.id} t={t} me={user.id} onSelect={setSelectedId} onToggleDone={toggleDone} onPatch={patchTask} partnerId={partnerId} />)}
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-purple-500/30 bg-panel p-4">
          {!selected && (
            <div className="space-y-2 text-sm text-purple-200">
              ...
              <p>おすすめ: 大きめタスクは「小さな行動」に分けると続きやすいです。</p>
            </div>
          )}
          {selected && (
            <div className="space-y-3">
              ...
              <label className="text-sm">
                タイトル
                <input
                  className="mt-1 w-full rounded-xl bg-bg px-3 py-2"
              ...
              <label className="text-sm">
                メモ
                <textarea
                  className="mt-1 w-full rounded-xl bg-bg px-3 py-2 min-h-28"
                  value={selected.memo ?? ''}
                  onChange={(e) => setTasks((prev) => prev.map((t) => (t.id === selected.id ? { ...t, memo: e.target.value } : t)))}
                />
              </label>
              <div className="grid grid-cols-2 gap-2">
                <label className="text-sm">
                  ...
                  <input
                    ...
                    onChange={(e) => setTasks((prev) => prev.map((t) => (t.id === selected.id ? { ...t, due_date: e.target.value || null } : t)))}
                  />
                </label>
                <label className="text-sm">
                  優先度
                  <select
                    ...
                    onChange={(e) => setTasks((prev) => prev.map((t) => (t.id === selected.id ? { ...t, priority: e.target.value as Priority } : t)))}
                  >
              ...
              <label className="text-sm">
                ...
                <select
                  ...
                  onChange={(e) => setTasks((prev) => prev.map((t) => (t.id === selected.id ? { ...t, assignee: e.target.value || null } : t)))}
                >
                  <option value="">未割当</option>
                  <option value={user.id}>わたし</option>
                  {partnerId && <option value={partnerId}>パートナー</option>}
                </select>
              </label>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button className="rounded-xl bg-card px-4 py-2" onClick={() => toggleDone(selected)}>
                  {selected.status === 'open' ? '完了にする' : '未完了に戻す'}
                </button>
              </div>
              <p className="text-xs text-purple-300">
                作成: {new Date(selected.created_at).toLocaleString()} / 更新: {new Date(selected.updated_at).toLocaleString()}
              </p>
            </div>
          )}
        </section>
  ...
  return (
    <article className="mb-2 rounded-xl border border-purple-500/25 bg-card p-3">
      <button onClick={() => onSelect(t.id)} className="w-full text-left">
        <p className="font-semibold text-purple-50">{t.title}</p>
      </button>
      <div className="mt-2 flex flex-wrap gap-2 text-xs text-purple-200">
        ...
        <span className="rounded-full bg-black/25 px-2 py-1">担当: {t.assignee === me ? 'わたし' : t.assignee === partnerId ? 'パートナー' : '未割当'}</span>
      </div>
      <div className="mt-2 flex gap-2 text-xs">
        <button className="rounded bg-black/25 px-2 py-1" onClick={() => onToggleDone(t)}>
          {t.status === 'open' ? '完了にする' : '未完了に戻す'}
        </button>
        <button
          className="rounded bg-black/25 px-2 py-1"
          onClick={() => onPatch(t.id, { assignee: t.assignee === me ? partnerId : me })}
        >
          担当切替
        </button>
        <button
          className="rounded bg-black/25 px-2 py-1"
          onClick={() => onPatch(t.id, { priority: t.priority === 'high' ? 'medium' : t.priority === 'medium' ? 'low' : 'high' })}
        >
          優先度変更
        </button>
      </div>
