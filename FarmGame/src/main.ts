...
interface ManifestCrop { id: string; sprout: string; mid: string; ready: string; }
interface ManifestData { crops?: ManifestCrop[] }
...
let textureByKey = new Map<string, Texture>();
...
function getGrowthStage(elapsedMs: number, T: number): GrowthStage {
  ...
  if (e < sproutMs) return 'sprout';
  if (e < sproutMs + midMs) return 'mid';
  return 'ready';
}

function getCropSpriteKey(cropId: string, stage: GrowthStage): string {
  if (stage === 'sprout') return `${cropId}_sprout`;
  if (stage === 'mid') return `${cropId}_mid`;
  return `${cropId}_ready`;
}

function clamp(s: SaveDataV1): SaveDataV1 {
  s.coins = Math.max(0, Math.floor(s.coins || 0));
  s.fuel = Math.max(0, Math.min(9999, Math.floor(s.fuel || 0)));
  s.gridRows = Math.max(3, Math.min(5, Math.floor(s.gridRows || 3)));
  s.gridCols = Math.max(4, Math.min(6, Math.floor(s.gridCols || 4)));
...
function load(): SaveDataV1 {
  try {
    const x = localStorage.getItem(SAVE_KEY);
    return x ? clamp(JSON.parse(x)) : defaultState();
  } catch {
    return defaultState();
  }
}
...
function plant(i: number) {
  ...
  const c = crop(state.ui.selectedCropId);
  if (!t || !c || t.state !== 'empty' || state.coins < c.seedCost) return;
  const now = Date.now();
  state.coins -= c.seedCost;
  state.grid[i] = { state: 'growing', cropId: c.id, plantedAt: now, readyAt: now + c.growMs };
}
...
function harvest(max: number) {
  ...
  const targets = state.grid
    .map((t, i) => ({ t, i }))
    .filter((x) => x.t.state === 'ready')
    .sort((a, b) => (a.t.readyAt || 0) - (b.t.readyAt || 0));
  const n = Math.min(max, targets.length, state.fuel);
  for (let i = 0; i < n; i++) {
    const c = targets[i].t.cropId!;
    state.warehouse[c] = (state.warehouse[c] || 0) + 1;
    state.grid[targets[i].i] = defaultTile();
  }
  state.fuel -= n;
  return n;
}
...
function sellAction() {
  ...
  const cap = SELL_CAP[state.cats.seller.level - 1];
  const minFuel = Math.ceil(cap / 10);
  if (state.fuel < minFuel) return 0;
  const list = Object.entries(state.warehouse)
    .filter(([, v]) => v > 0)
    .sort((a, b) => (crop(b[0])?.sellPrice || 0) - (crop(a[0])?.sellPrice || 0));
  ...
  for (const [id, qty] of list) {
    ...
    state.warehouse[id] -= take;
    remain -= take;
    sold += take;
    gain += take * (crop(id)?.sellPrice || 0);
  }
  if (!sold) return 0;
  state.fuel -= Math.ceil(sold / 10);
  state.coins += gain;
  return gain;
}
...
function applyOffline() {
  const now = Date.now();
  let elapsed = Math.max(0, now - state.lastSeen);
  elapsed = Math.min(elapsed, MAX_OFFLINE_MS);
  const steps = Math.floor(elapsed / 60000);
  let h = 0; let gain = 0; let fuelSpent = 0;
  for (let i = 1; i <= steps; i++) {
    const t = state.lastSeen + i * 60000;
    updateReady(t);
    const limit = Math.ceil(60 / HARVEST_INTERVAL[state.cats.harvester.level - 1]);
    const hh = harvest(limit);
    h += hh;
    fuelSpent += hh;
    for (let k = 0; k < 6; k++) {
...
function renderFarm() {
  ...
  for (let i = 0; i < state.grid.length; i++) {
    ...
    const base = new Graphics()
      ...
      .fill(t.state === 'ready' ? 0xB6E7F2 : t.state === 'growing' ? 0xF3F7CF : 0xF2D6B3)
      .stroke({ color: 0x000000, alpha: .15, width: 1 });
    base.eventMode = 'static';
    base.cursor = 'pointer';
    base.on('pointertap', () => {
      if (state.ui.tab !== 'farm') return;
      if (state.grid[i].state === 'ready') harvest(1); else plant(i);
      renderAll();
    });
    farmLayer.addChild(base);

    if (t.state !== 'empty' && t.cropId) {
      const c = crop(t.cropId);
      const elapsed = Date.now() - (t.plantedAt || Date.now());
      const stage = c ? getGrowthStage(elapsed, c.growMs) : 'ready';
      const key = getCropSpriteKey(t.cropId, stage);
      const tex = textureByKey.get(key);
      ...
      farmLayer.addChild(new Text({
        text: stage === 'ready' ? 'ready' : stage,
        style: new TextStyle({ fontFamily: 'Noto Sans JP', fontSize: 10, fill: 0x2B2B2B }),
        x: x + 16,
        y: y + 44,
      }));
    }
...
function renderTabs() {
  ...
  (['farm', 'shop', 'cats', 'settings'] as TabKey[]).forEach((t) => {
    ...
    if (state.ui.tab === t) b.classList.add('active');
    b.onclick = () => { state.ui.tab = t; renderAll(); };
...
function renderPanel() {
  ...
  if (tab === 'farm') {
    panelEl.innerHTML = `<div class="row" id="crops"></div><div class="row"><button id="plantAll" class="primary">まとめ植え</button></div><div class="muted">空いているマスにまとめて植えられるよ</div><div class="muted">収穫猫: ${state.cats.harvester.enabled && state.fuel > 0 ? '稼働' : '停止'} / 販売猫: ${state.cats.seller.enabled && state.fuel > 0 ? '稼働' : '停止'}</div>`;
    const wrap = panelEl.querySelector('#crops')!;
    unlocked().forEach((c) => {
      const b = document.createElement('button');
      b.textContent = `${c.name} ${Math.round(c.growMs / 1000)}s`;
      if (state.ui.selectedCropId === c.id) b.classList.add('active');
      b.onclick = () => { state.ui.selectedCropId = c.id; renderAll(); };
      wrap.appendChild(b);
    });
  ...
  if (tab === 'shop') {
    ...
    FUEL_PACKS.forEach((p) => {
      const b = document.createElement('button');
      b.textContent = `${p.n} ${p.f}燃料/${p.c}🪙`;
      b.onclick = () => { if (state.coins >= p.c) { state.coins -= p.c; state.fuel = Math.min(9999, state.fuel + p.f); renderAll(); } };
      fuel.appendChild(b);
    });
    panelEl.querySelector<HTMLButtonElement>('#expand')!.onclick = () => {
      const cost = GRID_COST[state.unlocks.gridExpansions];
      if (!cost || state.coins < cost) return;
      if (state.gridCols < 6) state.gridCols++;
      else if (state.gridRows < 5) state.gridRows++;
      else return;
      state.coins -= cost;
      state.unlocks.gridExpansions++;
      while (state.grid.length < state.gridRows * state.gridCols) state.grid.push(defaultTile());
      renderAll();
    };
    panelEl.querySelector<HTMLButtonElement>('#upH')!.onclick = () => {
      const lv = state.cats.harvester.level;
      const cost = CAT_COST[lv - 1];
      if (cost && state.coins >= cost && lv < 5) { state.coins -= cost; state.cats.harvester.level++; renderAll(); }
    };
    panelEl.querySelector<HTMLButtonElement>('#upS')!.onclick = () => {
      const lv = state.cats.seller.level;
      const cost = CAT_COST[lv - 1];
      if (cost && state.coins >= cost && lv < 5) { state.coins -= cost; state.cats.seller.level++; renderAll(); }
    };
  }
  ...
  if (tab === 'settings') {
    ...
    panelEl.querySelector<HTMLButtonElement>('#exp')!.onclick = () => {
    ...
    panelEl.querySelector<HTMLInputElement>('#imp')!.onchange = async (e) => {
      ...
      if (!f) return;
      try {
        const data = clamp(JSON.parse(await f.text()));
        const req = ['coins', 'fuel', 'lastSeen', 'gridRows', 'gridCols', 'grid', 'warehouse', 'cats', 'unlocks', 'settings', 'version'];
        if (!req.every((k) => k in data)) throw new Error('bad');
        state = data;
        renderAll();
      } catch {
        (panelEl.querySelector('#msg') as HTMLDivElement).textContent = '不正なJSONです';
      }
    };
    panelEl.querySelector<HTMLButtonElement>('#reset')!.onclick = () => {
      if (confirm('セーブをリセットしますか？')) { state = defaultState(); renderAll(); }
    };
  }
}

function renderStats() {
  statsEl.textContent = `🪙 ${state.coins} / 🐾 ${state.fuel}`;
}
...
async function loadManifestTextures() {
  try {
    const manifest = (await (await fetch('./assets/manifest.json')).json()) as ManifestData;
    for (const c of manifest.crops ?? []) {
      const entries: Array<[string, string]> = [
        [`${c.id}_sprout`, c.sprout],
        [`${c.id}_mid`, c.mid],
        [`${c.id}_ready`, c.ready],
      ];
      for (const [key, path] of entries) {
        try {
          ...
          textureByKey.set(key, tex);
        } catch {
          // keep fallback drawing
        }
      }
    }
  } catch {
